...
export function TeamHomePage() {
  ...
  // Rename pops the existing CreateOfficeModal in "rename" mode —
  // the modal is shape-compatible (name input, submit handler,
  // configurable label/title) so we don't need a second modal
  // primitive. The slug stays put; renameOffice in the repo only
  // touches `name` so existing share links + URLs keep working.
  const [renameTarget, setRenameTarget] = useState<OfficeListItem | null>(null)
  async function performRename(name: string) {
    if (!renameTarget) return
    const target = renameTarget
    const prev = offices
    // Optimistic — flip the name in the list immediately so the
    // dashboard reads correct before the server round-trips.
    setOffices((os) => os.map((o) => (o.id === target.id ? { ...o, name } : o)))
    setRenameTarget(null)
    try {
      await renameOffice(target.id, name)
    } catch (err) {
      console.warn('Rename office failed; restoring list', err)
      setOffices(prev)
  ...
  return (
    <div className="min-h-screen bg-[color:var(--color-paper)] dark:bg-gray-950">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-6 sm:py-8">
        ...
        {/* Rename modal reuses the CreateOfficeModal primitive. The
            slug is intentionally NOT regenerated by performRename so
            existing links + bookmarks keep working; the URL hint
            below tells the user that. */}
        {renameTarget && (
          <CreateOfficeModal
            onClose={() => setRenameTarget(null)}
            defaultName={renameTarget.name}
            title="Rename office"
            description="The URL stays the same — only the display name updates. Existing share links keep working."
            submitLabel="Rename"
