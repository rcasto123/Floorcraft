...
export function DepartmentChip({
  ...
  if (!department) {
    return (
      <span className="inline-flex items-center rounded-full bg-[color:var(--color-paper-sunken)] dark:bg-gray-800 text-gray-500 dark:text-gray-400 px-2.5 py-0.5 text-xs font-medium">
        —
      </span>
    )
  }
  ...
  if (!rgb) {
    return (
      <span className="inline-flex items-center rounded-full bg-[color:var(--color-paper-sunken)] dark:bg-gray-800 text-gray-600 dark:text-gray-300 px-2.5 py-0.5 text-xs font-medium">
        {department}
      </span>
    )
  }
