...
### Apply Database Migrations + Edge Functions

After every PR that adds a migration or touches an Edge Function:
...
This is the everyday flow. The script wraps `supabase db push --linked` for migrations and `supabase functions deploy` for the three Edge Functions Floorcraft expects (`send-invite-email`, `admin-send-password-reset`, `admin-set-user-suspension`). Idempotent — safe to re-run.

**One-time setup** (per workstation):

```bash
npx supabase link --project-ref <ref>   # caches DB pooler creds
npx supabase login                       # writes ~/.supabase/access-token (needed for function deploy)
```

**Fallback: paste the catchup SQL** (for fresh projects without the CLI, or ad-hoc cloud SQL editor work)

1. Open Supabase → SQL Editor → New query.
2. Paste [`scripts/catchup-admin-rpcs.sql`](scripts/catchup-admin-rpcs.sql) → Run.

The catchup script bundles every admin-side migration (currently 0018 → 0028) and is regenerated by `./scripts/build-catchup.sh` whenever a new migration lands. Idempotent end-to-end.

**Local dev**

```bash
npx supabase start    # spin up a local Supabase stack
...
Three Edge Functions back the team-invite and platform-admin surfaces:

| Function | Purpose | Needs |
|---|---|---|
| `send-invite-email` | Sends team-invite emails via Resend | `RESEND_API_KEY`, `APP_URL` |
| `admin-send-password-reset` | Generates a recovery link for the user-detail page | none beyond the auto-set service role |
| `admin-set-user-suspension` | Flips `auth.users.banned_until` for the suspend-user feature | none beyond the auto-set service role |

Deploy via the apply script (see [Apply Database Migrations + Edge Functions](#apply-database-migrations--edge-functions) above), or one-off:

```bash
npx supabase functions deploy <function-name>
```

Set Resend secrets once per project:

```bash
npx supabase secrets set RESEND_API_KEY=<your-key> APP_URL=https://floorcraft.space
```

`SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are populated automatically by Supabase.
