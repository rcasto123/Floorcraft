...
**Easy path: paste the catchup SQL** (no CLI required)

1. Open your Supabase project → SQL Editor → New query.
2. Paste the contents of [`scripts/catchup-admin-rpcs.sql`](scripts/catchup-admin-rpcs.sql) → Run.
3. Done. Re-paste anytime after a release that adds a migration — the file is regenerated in lock-step and is idempotent end-to-end.

The catchup script bundles every admin-side migration (currently 0018 → 0025) and is regenerated by `./scripts/build-catchup.sh` after any new migration lands.

**CLI path** (for ongoing local development)
